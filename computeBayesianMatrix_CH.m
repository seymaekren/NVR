function bayesianMatrix = computeBayesianMatrix_CH(ASSIGNTABLE, ...
...
%filenames{1} = 'allPeaksAssignmentEnvironment-For1UBI-forBayesianScoring-withNVR_ScoringMatrices-CH_RDCs.mat';
%    filenames{1} = 'allPeaksAssignmentEnvironment-For1UBI-forBayesianScoring-withNVR_ScoringMatrices-CH_RDCs-ExtraPeaks.mat';
...
%     load ('allPeaksAssignmentEnvironment-For1UBI-forBayesianScoring-withNVR_ScoringMatrices.mat');
      ...
      assert (size(SSCP,1) == numPeaks);
      assert (size(SSCP,2) == numResidues);
      
      rankOfCorrectAssignment = ones(numPeaks, numVoters);
	...
	for voterIndex = 1:numVoters
	  ...
	  for residueIndex = 1:numResidues
	    ...
	    if (voter{voterIndex}(peakIndex,residueIndex) > ...
		voter{voterIndex}(peakIndex,correctResidueIndex))
	      rankOfCorrectAssignment(peakIndex,voterIndex) = rankOfCorrectAssignment(peakIndex,voterIndex)+1;
	    end
	  end
	end
      end
      ...
      for voterIndex = 1:numVoters
	figure;
	plot(rankOfCorrectAssignment(1:numPeaks,voterIndex),'*')
	title('rank of correct assignment according to a voter');
      end
      keyboard
...
save bayesianMatrix.mat bayesianMatrix
rankOfThisPeak = ones(size(bayesianMatrix,1),1)*-1;
for relPeakIndex = 1:size(bayesianMatrix,1)
  correctAssignmentScore = -1; otherScores = [];
  for relResidueIndex = 1:size(bayesianMatrix,2)
    if (unassignedPeakIndices(relPeakIndex) == ...
	unassignedResidueIndices(relResidueIndex))
      correctAssignmentScore = bayesianMatrix(relPeakIndex, ...
					      relResidueIndex);
    else
      otherScores = [otherScores bayesianMatrix(relPeakIndex,relResidueIndex)];
    end
  end
  if (correctAssignmentScore ~= -1)
    largerEntries = find(otherScores > correctAssignmentScore);
    rankOfThisPeak(relPeakIndex) = length(largerEntries)+1;
  end
end
figure;
plot(rankOfThisPeak,'*');
title('rank of Bayesian score for correct assignment for relPeaks. -1 if rank undetermined');

copyOfBayesianMatrix = bayesianMatrix;
for i = 1:3
  maxValue = max(max(copyOfBayesianMatrix));
  [rowIndex,columnIndex] = find(copyOfBayesianMatrix == maxValue);
  copyOfBayesianMatrix(rowIndex,columnIndex) = 0;
  fprintf('assignment of %d to %d has score %f\n', ...
	  unassignedPeakIndices(rowIndex), ...
	  unassignedResidueIndices(columnIndex), maxValue);
end
