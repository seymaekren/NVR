function [MASTER, ASSIGNTABLE, CP, SXCP, SSCP, RP1, RP2, TP, HDE, ROWIN, COLIN,marsRdcScore,MB_RDC_Score,numRDC] = ...
...
% $$$ 
% $$$ 
% $$$ load confidentAssignments.txt
% $$$ numIncorrectAssignmentsAtStart = 0;
% $$$ numAssignmentsAtStart = length(confidentAssignments);
% $$$ 
% $$$ for i = 1:numAssignmentsAtStart
% $$$   peakIndex = confidentAssignments(i,1);
% $$$   absoluteResidueIndex = confidentAssignments(i,2);
% $$$   residueIndex  = find(RESNUMS  == absoluteResidueIndex);
% $$$   ASSIGNTABLE (peakIndex,:)            = 0;
% $$$   ASSIGNTABLE (peakIndex,residueIndex) = 1;
% $$$   if (peakIndex ~= residueIndex)
% $$$     numIncorrectAssignmentsAtStart = numIncorrectAssignmentsAtStart + 1;
% $$$   end
% $$$ end
% $$$ numCorrectAssignmentsAtStart = numAssignmentsAtStart - numIncorrectAssignmentsAtStart;
% $$$ 
% $$$ [MASTER,ASSIGNTABLE]=updateMASTER(MASTER,ASSIGNTABLE,ROWIN,COLIN);
% $$$ [ROWIN,COLIN]=sanitycheck(MASTER,ROWIN,COLIN);
% $$$ [ASSIGNTABLE,CP,SXCP,SSCP,RP1,RP2,TP,HDE,ROWIN,COLIN]= ...
% $$$     reduceAll(ASSIGNTABLE,CP,SXCP,SSCP,RP1,RP2,TP,HDE,ROWIN,COLIN);
% $$$ [ROWIN,COLIN]=sanitycheck(MASTER,ROWIN,COLIN);
% $$$ fprintf(1, 'loaded %d incorrect, %d correct assignments\n',numIncorrectAssignmentsAtStart,numCorrectAssignmentsAtStart);
% $$$ 
% $$$ sum(sum(MASTER))
% $$$ 
% $$$ 
% $$$ %jumping directly to the second phase.
% $$$ 
% $$$ fprintf(1,'jumping directly to the second phase.\n');
% $$$ 
% $$$ if (0)


% $$$ load origCP.mat
% $$$ figure; plot(origSSCP - SSCP);
% $$$ figure; plot(origSXCP - SXCP);
% $$$ figure; plot(origTP - TP);
% $$$ figure; plot(origHDE- HDE);
% $$$ figure; plot(origASSIGNTABLE - ASSIGNTABLE);
% $$$ figure; plot(origROWIN - ROWIN);
% $$$ figure; plot(origCOLIN - COLIN);
%keyboard
...
fprintf(1, 'assigned %d peaks at 1.\n',sum(sum(MASTER)));
...
fprintf(1, 'assigned %d peaks at 2.\n',sum(sum(MASTER)));
...
fprintf(1, 'assigned %d peaks at 3.\n',sum(sum(MASTER)));
%keyboard
for(i=[.3 .1])
   if(sum(sum(MASTER))>8)
      break;
...
fprintf(1, 'assigned %d peaks at 4.\n',sum(sum(MASTER)));
...
fprintf(1, 'assigned %d peaks at 5.\n',sum(sum(MASTER)));
...
fprintf(1, 'assigned %d peaks at 6.\n',sum(sum(MASTER)));
...
fprintf(1, 'assigned %d peaks at 7.\n',sum(sum(MASTER)));
...
for peakIndex=1:length(assignedPeakIndices)
  ...
  scores(peakIndex)    = scores(peakIndex)  - log (origSXCP(assignedPeakIndex, ...
						  assignedResidueIndex)) ;
  scores(peakIndex)    = scores(peakIndex)  - log(origTP(assignedPeakIndex, ...
						  assignedResidueIndex)) ;
...
fprintf(1, 'computing optimum mars rdc score.\n');
correctAssigntable = ASSIGNTABLE*0;
for peakIndex = 1:size(ASSIGNTABLE,1)
  correctAssigntable(peakIndex,peakIndex) = 1;
end


[S1,saupeWrongFlag]                               = updateTen(correctAssigntable,RDC1,VECTORS);
assert (saupeWrongFlag == 0);
[S2,saupeWrongFlag]                               = updateTen(correctAssigntable,RDC2,VECTORS);
assert (saupeWrongFlag == 0);
marsRdcScore                                      = computeMarsRDC_Score(correctAssigntable, RDC1, RDC2, ...
						  VECTORS, S1, S2);



fullyCorrectAssigntable = ASSIGNTABLE*0;
for peakIndex = 1:size(ASSIGNTABLE,1)
  fullyCorrectAssigntable(peakIndex,peakIndex) = 1;
end

[S1,saupeWrongFlag]                               = updateTen(fullyCorrectAssigntable,RDC1,VECTORS);
assert (saupeWrongFlag == 0);
[S2,saupeWrongFlag]                               = updateTen(fullyCorrectAssigntable,RDC2,VECTORS);
assert (saupeWrongFlag == 0);

[MB_RDC_Score,numRDC]                             = computeMB_RDC_Score (fullyCorrectAssigntable,RDC1, RDC2, VECTORS, ...
						  S1, S2);
...
[S1,saupeWrongFlag]                               = updateTen(ASSIGNTABLE,RDC1,VECTORS);
assert (saupeWrongFlag == 0);
...
% $$$ %---------------------------------------------------------------------
% $$$ %---------------------------------------------------------------------
% $$$ %I think this is the location where the execution takes on from RDC's.
% $$$ %---------------------------------------------------------------------
% $$$ %---------------------------------------------------------------------
% $$$ %---------------------------------------------------------------------
% $$$ %---------------------------------------------------------------------
...
% $$$ 
% $$$ 
% $$$ [S1 flag1]= updateTen(MASTER,RDC1,VECTORS);
% $$$ if(flag1==0)
% $$$    RP1 = NVR_RDC2PROB(ASSIGNTABLE,RDC1,VECTORS,S1);
% $$$ else
% $$$    RP1  = (ASSIGNTABLE*0+1);
% $$$ end
% $$$ 
% $$$ [S2 flag2] = updateTen(MASTER,RDC2,VECTORS);
% $$$ if(flag2==0)
% $$$    RP2 = NVR_RDC2PROB(ASSIGNTABLE,RDC2,VECTORS,S2);
% $$$ else
% $$$    RP2  = (ASSIGNTABLE*0+1);
...
% $$$ 
% $$$ 
% $$$ %HD_SCORE    = computeHD_Score( RDC1, RDC2, VECTORS,  ...
