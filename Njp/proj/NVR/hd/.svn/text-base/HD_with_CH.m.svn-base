function [HD_SCORE,assignmentAccuracy, assignments, weightedHD_Score] = HD_with_CH(peaks,NH_RDCS,CH_RDCS,HDEXCHANGE, ...
						  peakIDs, NOES, ...
						  VECTORS,VECTORS_CH,TYPES, ...
...
%				        NH_RDCS = a Nx1 matrix containing N-H RDCs in 1 medium
%                                   *** CH_RDCS = a Nx1 matrix containing CA-HA RDCs in 1 medium (the same medium as the N-H RDCs)
...
%					VECTORS = A Mx3 matrix containing normalized backbone amide bond vectors
%                                   *** VECTORS_CH = A Mx3 matrix containing normalized CA-HA bond vectors
...
addpath('~njp/code/JBN-Submission-Snapshot-06-15-07/Matlab/General');
...
RP2 = NVR_RDC2PROB(ASSIGNTABLE,RDC2,VECTORS_CH,S1);
...
for(i=[.3 .1])
   ...
   [MASTER,ASSIGNTABLE,CP,SXCP,SSCP,RP1,RP2,TP,HDE,ROWIN,COLIN]=combine(SXCP,HDE,MASTER,ASSIGNTABLE,CP,SXCP,SSCP,RP1,RP2,TP,HDE,ROWIN,COLIN,NOES,IALLDISTS,NTH,S1,RDC1,RDC2,VECTORS,VECTORS_CH,i);
...
[S1 flag1]= updateTen_CH(MASTER,RDC1,RDC2,VECTORS,VECTORS_CH);
if(flag1==0)
   RP1 = NVR_RDC2PROB(ASSIGNTABLE,RDC1,VECTORS,S1);
   RP2 = NVR_RDC2PROB(ASSIGNTABLE,RDC2,VECTORS_CH,S1);
else
...
fail=0;
for(i=[.1])
   [MASTER,ASSIGNTABLE,CP,SXCP,SSCP,RP1,RP2,TP,HDE,ROWIN,COLIN]=combine(RP1.*RP2,TP,MASTER,ASSIGNTABLE,CP,SXCP,SSCP,RP1,RP2,TP,HDE,ROWIN,COLIN,NOES,ALLDISTS,15,S1,RDC1,RDC2,VECTORS,VECTORS_CH,i);
   for(q=1:10)
      [ASSIGNTABLE] = lockdown(MASTER,ASSIGNTABLE,NOES,IALLDISTS, NTH,ROWIN,COLIN);
...
while(sum(sum(MASTER))<size(MASTER,1))
   ...
   for(i=[.4])
      [MASTER,ASSIGNTABLE,CP,SXCP,SSCP,RP1,RP2,TP,HDE,ROWIN,COLIN]=combine(SXCP,HDE,MASTER,ASSIGNTABLE,CP,SXCP,SSCP,RP1,RP2,TP,HDE,ROWIN,COLIN,NOES,ALLDISTS,15,S1,RDC1,RDC2,VECTORS,VECTORS_CH,i);
      for(q=1:10)
         [ASSIGNTABLE] = lockdown(MASTER,ASSIGNTABLE,NOES,IALLDISTS, NTH,ROWIN,COLIN);
         [ASSIGNTABLE] = lockdown(MASTER,ASSIGNTABLE,NOES,ALLDISTS, BTH,ROWIN,COLIN);
         [MASTER,ASSIGNTABLE]=updateMASTER(MASTER,ASSIGNTABLE,ROWIN,COLIN);[ROWIN,COLIN]=sanitycheck(MASTER,ROWIN,COLIN);
...
function  [MASTER,ASSIGNTABLE,CP,SXCP,SSCP,RP1,RP2,TP,HDE, ROWIN,COLIN]=combine(A,B,MASTER,ASSIGNTABLE,CP,SXCP,SSCP,RP1,RP2,TP,HDE, ROWIN,COLIN,NOES,ALLDISTS,NTH,S1,RDC1,RDC2,VECTORS,VECTORS_CH,THR);
THR;
if(THR>0)
   ...
   [MASTER,ASSIGNTABLE]=updateMASTER(MASTER,ASSIGNTABLE,ROWIN,COLIN);[ROWIN,COLIN]=sanitycheck(MASTER,ROWIN,COLIN);
   [ASSIGNTABLE,CP,SXCP,SSCP,RP1,RP2,TP,HDE, ROWIN,COLIN]=reduceAll(ASSIGNTABLE,CP,SXCP,SSCP,RP1,RP2,TP,HDE, ROWIN,COLIN);[ROWIN,COLIN]=sanitycheck(MASTER,ROWIN,COLIN);
   [S1,flag] = updateTen_CH(MASTER,RDC1,RDC2,VECTORS,VECTORS_CH);
   RP1 = NVR_RDC2PROB(ASSIGNTABLE,RDC1,VECTORS,S1);
   RP2 = NVR_RDC2PROB(ASSIGNTABLE,RDC2,VECTORS_CH,S1);
end
