...
%  keyboard
  ...
  noes = load('NOES.m');
  CH_RDCS = load('data/C-H_medium1.m');
  NH_RDCS = load('data/N-H_medium1.m');
  VECTORS_CH = load('data/C-H_vectors.m');
  VECTORS_NH = load('data/N-H_vectors.m');
end
...
RDC2 = CH_RDCS(:,2);
VECTORS = VECTORS_NH;
completeMASTER = zeros(65,65);
for i = 1:65
  completeMASTER(i,i) = 1;
end
S1 = updateTen_CH(completeMASTER,RDC1,RDC2,VECTORS(:,2:4),VECTORS_CH(:,2:4));
...
for i = 1:length(NH_RDCS)
  if (NH_RDCS(i,2) ~= -999)
    plot(NH_RDCS(i,2),backcomputed(i),'*');
    hold on
  else
...
for i = 1:length(CH_RDCS)
  if (CH_RDCS(i,2) ~= -999)
    plot(CH_RDCS(i,2),backcomputed(i),'*');
    hold on
  else
    fprintf(1, 'skipping %d th RDC.\n',i);
  end
...
%[assignmentAccuracy, HD_SCORE] = HD_with_CH(HSQCDATA(:,2:3),NH_RDCS(:,2),CH_RDCS(:,2),HSQCDATA(:,6),HSQCDATA(:,1),NOES,VECTORS_NH(:,2:4),VECTORS_CH(:,2:4),TYPES,RESNUMS,SSTRUCT, HBOND, ALLDISTS,IALLDISTS,SHIFTS_Filename,SHIFTX_Filename)
