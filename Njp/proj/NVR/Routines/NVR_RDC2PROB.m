...
function M = NVR_RDC2PROB(TABLE,RDC,VECS,TEN);
...
for(i=1:size(M,1))
   if(RDC(i)==-999)%means there was no RDC, give it a uniform probability over possible assignments
      M(i,:) = 1.0*TABLE(i,:);
   else
   ...
   for(j=1:size(M,2))
      %compute the likelyhood of seeing the observed RDC given the vector 
         %and the tensor
         if(TABLE(i,j)>0)
            M(i,j) = normpdf(RDC(i)-backcomputed(j),0,range(backcomputed)/8);
         end
      end
   end
   if(sum(M(i,:))>0)
