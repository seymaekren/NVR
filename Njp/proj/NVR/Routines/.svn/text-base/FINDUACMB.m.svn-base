function M = FINDUACMB(A,B,ASSIGNTABLE,THR)

%FINDUACMB: 

%Input parameters: 
%
%A and B are two bipartite graphs. THR is a threshold.
%ASSIGNTABLE contains the current state of the possible remaining
%assignments.
%
%This function computes the assignments using the Hungarian
%algorithm for A and B separately. It filters these assignments
%using ASSIGNTABLE and also by thresholding with THR in A and B.
%The remaining possible assignments are returned in M.
%I do not know what FINDUACMB stands for, but I think it
%corresponds to something like: "find unambiguous common bipartite
