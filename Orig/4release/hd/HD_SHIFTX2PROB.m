function [M] = HD_SHIFTX2PROB(TABLE,H,N,TYPES,SSTRUCT,NOES,ALLDISTS,NTH,ROWIN,COLIN);

%NVR_HD2PROB: This computes assignment probabilities based on the program SHIFTX, it is not meant
...
%//  Version:		0.1
%//
%//  Description:	 This computes assignment probabilities based on on the program SHIFTX
%//
%// authors:
...
[rn TY SS ha hn nf cb ca co]= textread('SHIFTX.m','%f %s %s %f %f %f %f %f %f \n');
...
if(strcmp(SSTYPE,'C')==1)
   ...
   xHMX = FILTERS(3,TY,1,4)*5;
   xHMN = FILTERS(3,TY,1,3)*5;
   xNMX = FILTERS(3,TY,2,4)*3;
   xNMN = FILTERS(3,TY,2,3)*3;
...
elseif(strcmp(SSTYPE,'B')==1)
   ...
   xHMX = FILTERS(2,TY,1,4)*21;
   xHMN = FILTERS(2,TY,1,3)*21;
   xNMX = FILTERS(2,TY,2,4)*3;
   xNMN = FILTERS(2,TY,2,3)*3;
   ...
   xHMX = FILTERS(1,TY,1,4)*4;
   xHMN = FILTERS(1,TY,1,3)*4;
   xNMX = FILTERS(1,TY,2,4)*5;
   xNMN = FILTERS(1,TY,2,3)*5;
