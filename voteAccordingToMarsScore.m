function  [relCandidatePeakIndices, relCandidateResidueIndices, scores, numCandidates] =  voteAccordingToMarsScore(voter, numVoters, ASSIGNTABLE,peakIndices,residueIndices);
...
for relPeakIndex = 1:size(V,1)
  for relResidueIndex = 1:size(V,2)
    if (V(relPeakIndex,relResidueIndex) == 0)
      fprintf(1, 'replacing a 0 entry.\n');
      V(relPeakIndex,relResidueIndex) = LARGE_VALUE;
    end
  end
end
...
%while (~stopGeneratingCandidates)
  ...
  maxCombinedProbability  = min(min(V));
  
  if (maxCombinedProbability == LARGE_VALUE)
    break;
  end
  ...
  for (i = 1:numVoters)
    ...
    scores(numCandidates) = scores(numCandidates) +	voter_i_vote;
  end
...
% $$$   end
  ...
  V(bestRelPeakIndex, bestRelResidueIndex) = LARGE_VALUE;
end
